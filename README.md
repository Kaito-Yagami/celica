...
tools/export_api.py     writes that API out as static JSON under docs/api/
...
— and writes `docs/assets/fig/*.png` plus `docs/data/manual.js`. Flags: `--curated` renders
...
faces and edges with uint16, and base64s all of it into `docs/data/model.js` — about 590 KB.
three.js is vendored at `docs/assets/js/vendor/three.min.js` for the same reason.
...
matching `docs/assets/fig/pNNNN.png`.
...
```bash
python tools/export_api.py      # writes docs/api/, ~2,040 files, 2.6 MB
```

Every endpoint becomes a file. No server, no Node at runtime, works from `file://` and from
any static host. **This is the form a cloud service can actually reach** — put `docs/` on
